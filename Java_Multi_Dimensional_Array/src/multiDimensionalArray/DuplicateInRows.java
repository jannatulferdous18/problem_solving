...
public class DuplicateInRows {
	...
	public static void main(String[] args) {
		...
		findDuplicateEachRow(twoD_arr, row, column);
	}

	private static void findDuplicateEachRow(int[][] twoD_arr, int row, int column) {
		...
		System.out.println("After replacing duplicates in each row by zero: ");
		for (int i = 0; i < twoD_arr.length; i++) {
			for (int j = 0; j < column; j++) {
				System.out.print(twoD_arr[i][j] + "    ");
			}
